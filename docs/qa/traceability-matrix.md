...
| REQ-005 | As an on-call responder, I want to acknowledge, mute, and resolve incidents so that the team has clear operational ownership during an outage. | [#5](https://github.com/IDNTEQ/site-monitor/issues/5) | Incident action UI plus `/api/incidents/:incidentId/actions` contract | `npm test` | `public/index.html`, `public/app.js`, `public/styles.css`, `src/domain/incidents.js`, `src/domain/seed.js`, `src/server.js`, `src/store.js`, `test/incidents.test.js`, `test/api.test.js` | In Progress |
